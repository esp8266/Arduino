...
void ICACHE_RAM_ATTR pwm_timer_isr(){//103-138
	...
	if(current_step < table->len){//20/21
		if(table->masks[current_step] & 0xFFFF) GPOC = table->masks[current_step] & 0xFFFF;//15/21
		if(table->masks[current_step] & 0x10000) GP16O = 0;//6/13
		current_step++;//1
	} else {
		current_step = 0;//1
		if(pwm_mask == 0){//12
		...
		if(pwm_mask & 0xFFFF) GPOS = pwm_mask & 0xFFFF;//11
		if(pwm_mask & 0x10000) GP16O = 1;//5/13
