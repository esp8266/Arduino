...
private:
  ...
  // Inline helpers
  inline void SDA_LOW()  { GPES = (1 << twi_sda); }
  inline void SDA_HIGH() { GPEC = (1 << twi_sda); }
  inline bool SDA_READ() { return (GPI & (1 << twi_sda)) != 0; }
  inline void SCL_LOW()  { GPES = (1 << twi_scl); }
  inline void SCL_HIGH() { GPEC = (1 << twi_scl); }
  inline bool SCL_READ() { return (GPI & (1 << twi_scl)) != 0; }
...
unsigned char Twi::writeTo(unsigned char address, unsigned char * buf, unsigned int len, unsigned char sendStop) {
  unsigned int i;
  if(!write_start()) return 4;//line busy
  if(!write_byte(((address << 1) | 0) & 0xFF)) {
    if (sendStop) write_stop();
